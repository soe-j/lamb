...
class Lambda {
  constructor(funcFolderPath, env) {
    ...
    // update config
    this.config.Environment.Variables.NODE_ENV = this.env;

    if (this.env === 'production') {
      this.config.FunctionName = this.config.FunctionName;
    } else {
      this.config.FunctionName = `${this.config.FunctionName}-${this.env}`;
    }
  }
